...
**Verified:** 2026-09-10 (P1–P7: Mission Ops, Mission Control, Console, Governed Streaming, Harness Expansion, Agent Workspace, Trace Center + Consolidation; P8.1: Instruction Engine foundation; P8.2: Context Selection; P8.3: PrivacyFirewall Activation; P8.4: Governed Model Adaptation)
...
| Instruction engine (QIE) | PARTIAL | P8.1–P8.4 shipped (ADR 0042): `src/instruction/` holds the canonical InstructionPlan contract (11 layers, 13 context categories, 8 trust classes), fail-closed category↔trust validation, documented trust precedence, char-based budget, output-contract/failure-policy representation, pure deterministic composer with canonical-JSON digest, P8.2 deterministic context selector, P8.3 firewall admission (governance lanes keyed on trust class, ADR 0041 classifier reused for sensitive-content rejection, zero I/O), and P8.4 governed model adaptation (pure translator ComposedInstruction → existing ModelRequest with digest/identity preserved in metadata; dispatch through the existing GovernedModelRuntime, no ungoverned fallback; `src/models/*` and `src/providers/*` untouched). Remaining QIE phases: P8.5 injection-defense enforcement, P8.6 harness scoring, P8.7 observability, P8.8 Studio/CLI inspection, P8.9 research/SDK. |
