...
| Check | Result |
| --- | --- |
| **P12 Secure Execution & Isolation gate, 2026-09-12** | **PASS: focused suites — `dist/runtime/mission-lifecycle/execution-policy.test.js` 15/15 (policy determinism + digest stability; risk-tier/descriptor-only timeout derivation; fail-closed unknown risk/malformed inputs; isolation honesty — IN_PROCESS = POLICY_RESTRICTED never a sandbox claim, missing backend/level = FAILED_CLOSED no downgrade; serialization round-trip + unknown-field/digest-tamper rejection; enforced-memory claim rejection; classification matrix — success never auto-VERIFIED; journal-state mapping; output containment drop-without-preview; journal reserve/settle terminal, orphan DISPATCHING→AMBIGUOUS never re-executed, durable cross-instance duplicate refusal, forged-record identity fail-closed). `dist/runtime/mission-lifecycle/secure-execution.security.test.js` 26/26 (S1-S26 adversarial matrix: model-supplied timeout/sandbox/fs/network/credential escalation rejected at parser shape; requestedCapabilities recorded-never-honored; policy-injection intent stays data; unauthorized capability denial end-to-end; capability substitution single-dispatch; forged approval never grants/dispatches; forged verification fails digest; timeout/cancellation/crash/duplicate replay never re-dispatched; oversized output contained + payload fail-closed; serialized-policy injection rejected; DATA-as-authority denied; extension id = unknown capability; governed chain provable via events; executeTool-missing harness fails closed; unavailable required isolation DENIED with durable event; backend-without-level never downgrades; concurrency exhaustion fails closed with structured code; cross-mission step isolation + independent execution; journal tamper fail-closed). Mission-lifecycle+actions 125/125 incl. P11 suites preserved. SDK 10/10 incl. P12 contract exports; SDK typecheck pass. Server/Studio/CLI 56/56 incl. P12 executionState/isolation wire fields + SSE liveTypes. Full regression: see the ordinary/serial rows below.** |
